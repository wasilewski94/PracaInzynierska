...
#include <linux/time.h> //to get timestamp
...
void adc_complete(void * context)
{
	if(atomic_dec_and_test(&adc_flag)) {
		if(dev->rx[0] != 0){
			...
			//rozkladam 64-bitowy ts na 8 bajtow
			int i=0;
			if(debug)
				printk(KERN_ALERT "dev->now: %llu",dev->now );
			for(i=0; i<8; i++){
				dev->buffer[(dev->head)+i+2] = (unsigned char)(dev->now >> (56-i*8));
			}					
			...
			if(debug)
				printk(KERN_ALERT "dodano do bufora 10 bajtow");
		}
	} 
}
...
enum hrtimer_restart adc_timer_proc(struct hrtimer *my_timer)
{
	if(atomic_dec_and_test(&hrtimer_flag)) {  
		...
		//timestamp
		getnstimeofday(dev->timestamp);
		dev->now = timespec_to_ns(dev->timestamp);
...
static int kw_adc_init(void)
{
	...
	dev->timestamp = kmalloc(sizeof(struct timespec), GFP_KERNEL);
	if (!dev->timestamp)
		goto err1;	
