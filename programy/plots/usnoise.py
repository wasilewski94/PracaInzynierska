...
#generujemy szum na poziomie < 50us
noise = np.random.rand(N)/20000
x_noised = x + noise
...
axes[0].set_title(u'Sygnał wygenerowany numerycznie z nałożonym rozrzutem czasu próbkowania')
axes[0].plot(x_noised[1:1000],y_noised[1:1000], marker='.')

axes[1].set_title(u'Charakterystyka częstotliwościowa sygnału z rozrzutem czasu próbkowania')
axes[1].semilogy(xf[1:N//2], 2.0/N * np.abs(yf_noised[1:N//2]), '-b')

axes[2].set_title(u'Charakterystyka częstotliwościowa sygnału bez zniekształceń')
...
fig, axes = plt.subplots(3,1)
axes[0].set_title(u'Sygnał spróbkowany przetwornikiem ADC MAX1202')
axes[0].plot(t[1:1000],dane[1:1000],'-r', marker='.')

dane_fourier = fft(dane*hanning(len(dane)))
tf = np.linspace(0.0, 1.0/(2.0*T), N/2)

axes[1].set_title(u'Charakterystyka częstotliwościowa sygnału spróbkowanego z przetwornika MAX1202')
